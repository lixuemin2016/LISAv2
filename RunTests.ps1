...
try {
    ...
    if ($TotalVMs -eq 1) {
        ...
        $CUSTOM_VMS = $CUSTOM_VMS.Replace("EXTRA_NICS", "$($TotalNICsPerVM -1)")
        $CUSTOM_VMS = $CUSTOM_VMS.Replace("VM_NAME", "$RGIdentifier")
        $CUSTOM_VMS = $CUSTOM_VMS.Replace("VM_SIZE", "$OverrideVMSize")
    }
    else {
        $CUSTOM_VMS = ""
        $StartPort = 1111
        for ($j = 1; $j -le $TotalVMs; $j++ ) {
            $CurrentVM = $SingleVM
            ...
            $CurrentVM = $CurrentVM.Replace("EXTRA_NICS", "$($TotalNICsPerVM - 1)")
            $CurrentVM = $CurrentVM.Replace("VM_SIZE", "$OverrideVMSize")
            $CurrentVM = $CurrentVM.Replace("CUSTOM_ENDPOINTS", "$CUSTOM_ENDPOINTS")
            $CurrentVM = $CurrentVM.Replace("VM_NAME", "$RGIdentifier-$j")
            $CUSTOM_VMS += $CurrentVM
        }
    }
    $CUSTOM_RGS = $SingleResourceGroup.Replace("CUSTOM_VMS", "$CUSTOM_VMS")
    $CUSTOM_RGS = $CUSTOM_RGS.Replace("SingleVM","Deploy$TotalVMs`VM")
    $SmokeTestsConfigurations = (Get-Content .\XML\VMConfigurations\SmokeTestsConfigurations.xml)
    $SmokeTestsConfigurations = $SmokeTestsConfigurations.Replace('<!--CUSTOM_DEPLOYMENT-->', $CUSTOM_RGS)
    Set-Content -Value $SmokeTestsConfigurations -Path .\XML\VMConfigurations\SmokeTestsConfigurations.xml -Force

    $SmokeTests = (Get-Content .\XML\TestCases\SmokeTests.xml)
    $SmokeTests = $SmokeTests.Replace('SingleVM', "Deploy$TotalVMs`VM")
    Set-Content -Value $SmokeTests -Path .\XML\TestCases\SmokeTests.xml -Force
